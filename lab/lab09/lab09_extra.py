...
def reverse_other(t):
    ...
    def reverse(child, depth):
        if child.is_leaf():
            return
        if depth % 2 == 1:
            for i in range(len(child.branches)//2):
                child.branches[i].label, child.branches[-i -
                                                        1].label = child.branches[-i-1].label, child.branches[i].label
