...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("formals' length should be equal to vals' length")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        # Pair(symbol, Pair(Pair(args, nil)))
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        # Pair(Pair(name, formals), body)
        # target = Pair(name, formals)
        name = target.first
...
def do_if_form(expressions, env):
    ...
    # Pair(pred, Pair(cons, nil or Pair(alter, nil)))
...
def make_let_frame(bindings, env):
    ...
    curr_binding = bindings
    while curr_binding is not nil:
        binding = curr_binding.first # Pair(formal, Pair(expr, nil))
        check_form(binding, 2, 2)

        formal, expr = binding.first, binding.second
        if not scheme_symbolp(formal):
            raise SchemeError(f'{formal} is not a Symbol')
        
        val = scheme_eval(expr.first, env)
        formals, vals = Pair(formal, formals), Pair(val, vals)
        curr_binding = curr_binding.second
...
def do_define_macro(expressions, env):
    ...
    name, formals = target.first, target.second
    body = expressions.second
