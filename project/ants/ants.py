...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while next_place != hive:
            if cnt >= self.min_range and cnt <= self.max_range:
                if next_place.bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(armor)
        self.is_true = not QueenAnt.has_queen
    ...
    def action(self, colony):
        ...
        if not self.is_true:
            ...
            return
        super().action(colony)
        nxt_place = self.place.exit

        def buffer(ant):
            if ant and ant not in self.buffed_ants:
                ant.damage *= 2
                self.buffed_ants.append(ant)

        while nxt_place is not None:
            ant = nxt_place.ant
            if ant:
                buffer(ant)
                if ant.is_container:
                    buffer(ant.contained_ant)
            nxt_place = nxt_place.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    effected_action = effect(bee.action, bee)
