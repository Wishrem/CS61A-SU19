...
def taxicab(a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    diff_street = abs(street(a) - street(b))
    diff_avenue = abs(avenue(a) - avenue(b))
    return diff_street + diff_avenue
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        left_w, right_w =  total_weight(end(left_side)), total_weight(end(right_side))
        left_len, right_len = length(left_side), length(right_side)
        return left_w*left_len == right_w*right_len and balanced(end(left_side)) and balanced(end(right_side))
...
def mul_church(m, n):
    ...
    "*** YOUR CODE HERE ***"
    res = n
    for _ in range(church_to_int(m)-1):
